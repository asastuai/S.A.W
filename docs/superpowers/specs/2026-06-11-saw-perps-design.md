...
|---|---|---|
| Venue | **Adrena (devnet, v1) → Jupiter Perps (mainnet, prod)** — pivot #2, 2026-06-11; antes Drift | **Drift quedó descartado**: exploit de ~$285-295M el 1-abril-2026 (UNC4736/DPRK, ingeniería social al Security Council + durable nonces), exchange suspendido, programa mainnet reducido a shim admin, devnet congelado, relaunch sin fecha firme. Verificado on-chain (0/25 txs recientes lo invocan) + research con fuentes primarias. Adrena: vivo, SDK TS, devnet confirmado → preserva el zero-risk e2e de v1. Jupiter Perps: $660M TVL, permissionless, relación Jupiter ya existente por spot → producción post-funding. El adapter abstrae el venue (interfaz `VenueAdapter`), blast radius = 1 módulo. Sigue siendo wallet-signed, cero API keys. **Nota pitch**: el hack de Drift fue falla de policy/custodia admin — exactamente la clase de falla que SAW previene. |
