...
export function BellToggle() {
  ...
  useEffect(() => {
    ...
    setState(p);
    // A returning user who already granted: make sure THIS browser is
    // subscribed for closed-tab push too (best-effort, no-op if unsupported
    // or VAPID not configured).
    if (p === "granted" && pushSupported()) {
      void enablePush(getAccessToken);
    }
  ...
  async function onClick() {
    ...
    setState(next);
    // Once granted, also subscribe this browser to closed-tab web push.
    if (next === "granted") {
      void enablePush(getAccessToken);
    }
  }
  ...
  return (
    <button
      ...
      title={
        granted
          ? "Alerts on — sound + a native notification when your agent spots a move, even with the tab closed (installed PWA)"
          : denied
          ? "Notifications blocked in your browser. Sound still plays."
          : "Turn on alerts — sound + a native notification when your agent spots a move, even with the tab closed"
      }
